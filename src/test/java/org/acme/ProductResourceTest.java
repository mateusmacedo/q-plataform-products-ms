...
class ProductResourceTest {
    ...
    private void postAndExpect(String json, int status, String... expectedContents) {
        var req = given().contentType(MediaType.APPLICATION_JSON).body(json).when().post(BASE_URL).then()
                .statusCode(status);
        for (String content : expectedContents)
            req.body(containsString(content));
    }
    ...
    void shouldReturnErrorWhenNameIsTooLong() {
        postAndExpect(toJson(new ProductInputDTO(SKU_VALID, "Product 1234567890123456789012345678901234567890")), 400,
                NAME_LENGTH);
    }
    ...
    void shouldReturnErrorWhenSkuAlreadyExists() {
        ...
        postAndExpect(toJson(new ProductInputDTO(SKU_VALID, "Outro Nome")), 409,
                "Já existe um produto com o SKU " + SKU_VALID);
    }
    ...
    void shouldReturnErrorWhenNameAlreadyExists() {
        ...
        postAndExpect(toJson(new ProductInputDTO("SKU-UNICO-2", "NomeUnico")), 409,
                "Já existe um produto com o SKU SKU-UNICO-2");
    }

    @Test
    void shouldReturnErrorWhenGetNonexistentSku() {
        given().when().get(BASE_URL + "/SKU_INEXISTENTE").then().statusCode(404)
    ...
    void shouldReturnProductBySku() {
        postAndExpect(toJson(new ProductInputDTO(SKU_VALID, NAME_VALID)), 201, SKU_VALID, NAME_VALID);
        given().when().get(BASE_URL + "/" + SKU_VALID).then().statusCode(200).body(containsString(SKU_VALID))
                .body(containsString(NAME_VALID));
    }
    ...
    void shouldReturnXTraceIdHeaderInResponse() {
        ...
        given().contentType(MediaType.APPLICATION_JSON).header("X-Trace-Id", traceId).body(json).when().post(BASE_URL)
                .then().statusCode(201).header("X-Trace-Id", traceId);
    }

    @Test
    void shouldReturnInternalServerErrorOnMalformedJson() {
        String malformedJson = "{\"sku\":\"12345\",\"name\":\"Produto\""; // falta fechar o JSON
        given().contentType(MediaType.APPLICATION_JSON).body(malformedJson).when().post(BASE_URL).then().statusCode(500)
                .body(containsString("Erro interno do servidor"));
    }
}
